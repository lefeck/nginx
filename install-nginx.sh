...
printf "clear all environmentsn"
...
if [[ $(id nginx) = "0" ]]; then
    printf "nginx group and user is existn"
else
    groupadd -r nginx
...
sed -i 's/##BIN_PATH##/'${BIN_PATH}'/g' /etc/init.d/nginx
sed -i 's/##NGINX_PATH##/'${NGINX_PATH}'/g' /etc/init.d/nginx
